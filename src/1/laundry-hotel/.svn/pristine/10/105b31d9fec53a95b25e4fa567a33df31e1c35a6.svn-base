...
namespace ETexsys.Common.Sync
{
    public class WinRarHelper
    {
        ...
        public static bool CompressFilesToRar(List<string> sourceFilesPaths, string compressFileSavePath, string compressFileName)
        {
            //Process[] processes = Process.GetProcessesByName("WinRAR");
            //try
            //{
            //    foreach (Process p in processes)
            //    {
            //        TimeSpan ts = DateTime.Now - p.StartTime;
            //        if ((!p.Responding || ts.Minutes >= 5) && _winRarPath == p.MainModule.FileName)
            //        {
            //            p.Kill();
            //            p.Close();
            //        }
