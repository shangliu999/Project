...
namespace ETexsys.WebApplication.Areas.Dictionary.Controllers
{
    ...
    public class CategoryController : Controller
    {
        ...
        public ActionResult GetCategoryList(ParamModel model)
        {
            var query = i_category.Entities.Where(v => v.IsDelete == false);
        ...
        public ActionResult GetCategory(int cateId)
        {
            MsgModel msgModel = new MsgModel();

            category model = i_category.GetByKey(cateId);
        ...
        public JsonResult AddCategory(category requestParam)
        {
            MsgModel msgModel = new MsgModel();
            ...
            int rtn = i_category.Insert(requestParam);
        ...
        public JsonResult UpdateCategory(category requestParam)
        {
            MsgModel msgModel = new MsgModel();

            category model = i_category.GetByKey(requestParam.ID);
            ...
            if (model != null)
            {
                ...
                int rtn = i_category.Update(model);
        ...
        public JsonResult DelCategory(int cateId)
        {
            MsgModel msgModel = new MsgModel();
        ...
        public JsonResult CheckCateName(string CateName, int CateID)
        {
            ...
            if (CateID > 0)
            {
                int count = i_category.LoadEntities(v => v.ID != CateID && v.CateName == CateName && v.IsDelete == false).Count();
                result = count > 0 ? false : true;
            }
            else
            {
                int count = i_category.LoadEntities(v => v.CateName == CateName && v.IsDelete == false).Count();
                result = count > 0 ? false : true;
            }
