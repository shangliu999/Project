...
namespace ETexsys.WebApplication.Areas.Customer.Controllers
{
    ...
    public class ReceivingController : Controller
    {
        ...
        public ActionResult GetReceivingList(ParamModel model)
        {
            int hotelId = 0;
            ...
            var query = i_invoice.Entities.Where(v => v.InvType == 1&&v.DataType==2);
            if (hotelId != 0)
            {
        ...
        public ActionResult GetReceivingById(string bid)
        {
        ...
        public ActionResult Create(string id)
        {
            ...
            data.ToList().ForEach(c =>
            {
                ...
                if (details != null)
                {
                    ...
                    if (hotelid == 0 || regionid == 0)
                    {
                        ...
                        invSubType = details.FirstOrDefault().InvSubType;
                        date = details.FirstOrDefault().InvCreateTime;
                    }
                }
            ...
            ViewData["InvSubType"] = invSubType;
            return View(list);
        }

        public ActionResult Add(ReceivingModel model)
        {
            MsgModel msgModel = new MsgModel();

            string px = GetInvoicePX(1);
            int nubmer = GetInvoiceNubmer();
            string invNo = string.Format("{0}{1}{2}", px, model.InvCreateTime.ToString("yyyyMMdd"), nubmer.ToString().PadLeft(6, '0'));
            DateTime time = DateTime.Parse(model.InvCreateTime.ToString("yyyy-MM-dd"));
            ...
            inv.InvNo = invNo;
            inv.InvType = 1;
            inv.InvSubType = model.InvSubType;
            inv.TaskType = 0;
            inv.DataType = 2;
            ...
            ReceivingDetailsModel item = null;
            for (int i = 0; i < model.Data.Count; i++)
            {
                ...
                detailModel.InvSubType = model.InvSubType;
        ...
        public ActionResult Update(ReceivingModel model)
        {
            MsgModel msgModel = new MsgModel();
            ...
            if (invModel != null)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < model.Data.Count; i++)
                    {
                        ...
                        detailModel.InvType = 1;
                        detailModel.InvSubType = invModel.InvSubType;
                        detailModel.DataType = 2;
                        ...
                        detailModel.SizeName = item.SizeID == 0 ? "" : item.SizeName.Trim('(', ')');
                        detailModel.TextileCount = int.Parse(item.Count);
                        detailModel.InvCreateTime = invModel.CreateTime;
                        detailModel.BrandID = brandModel == null ? 0 : brandModel.ID;
                        detailModel.BrandName = brandModel == null ? "" : brandModel.BrandName;

                        detailList.Add(detailModel);
                    }
                    i_invoicedetail.Delete(c => c.InvID == invModel.ID);

                    i_invoicedetail.Insert(detailList);
                    invModel.Quantity = model.Quantity;
                    invModel.UpdateTime = model.InvCreateTime;
        ...
        public ActionResult DelReceiving(string bid)
        {
            MsgModel msgModel = new MsgModel();
