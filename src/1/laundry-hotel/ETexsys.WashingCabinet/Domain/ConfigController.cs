...
namespace ETexsys.WashingCabinet.Domain
{
    public class ConfigController
    {
        ...
        public static ReaderModel ReaderConfigDirt { get; set; }
        ...
        public static void Init()
        {
            ...
            ReaderConfigDirt = GetReaderConfigDirt();
        ...
        public static async void Login()
        {
            ...
            login.LoginPwd = "21218cca77804d2ba1922c33e0151105";
            login.LoginName = "成品操作员";
            login.TimeStamp = ApiController.Instance.GetTimeStamp();
            login.UUID = ConfigController.MacCode;
            login.TerminalType = ConfigController.TerminalType;
            var Login = await ApiController.Instance.DoLogin(ApiController.Instance.Login, login);
        }

        public static ReaderModel GetReaderConfigDirt()
        {
            ...
            try
            {
                if (File.Exists(path))
                {
                    string[] items = INIOperation.INIGetAllItems(path, "Reader1");
                    if (items.Length == 12)
                    {
                        reader = new ReaderModel();
                        string type = INIOperation.INIGetStringValue(path, "Reader1", "Type", "");
                        switch (type)
                        {
                            case "Alien":
                        ...
                        string connType = INIOperation.INIGetStringValue(path, "Reader1", "ConnType", "");
                        switch (connType)
                        {
                            case "TCPIP":
                        ...
                        reader.IPAddress = INIOperation.INIGetStringValue(path, "Reader1", "IPAddress", "");
                        reader.HostIPAddress = INIOperation.INIGetStringValue(path, "Reader1", "HostIPAddress", "");
                        reader.COM = INIOperation.INIGetStringValue(path, "Reader1", "COM", "");
                        reader.AntennaPower = Convert.ToInt32(INIOperation.INIGetStringValue(path, "Reader1", "AntennaPower", "0"));
                        reader.LoginName = INIOperation.INIGetStringValue(path, "Reader1", "LoginName", "");
                        reader.LoginPwd = INIOperation.INIGetStringValue(path, "Reader1", "LoginPwd", "");
                        reader.Frepuency = Convert.ToDouble(INIOperation.INIGetStringValue(path, "Reader1", "Frepuency", "0.0"));
                        reader.ReaderMode = INIOperation.INIGetStringValue(path, "Reader1", "ReaderMode", "");
                        string antenna = INIOperation.INIGetStringValue(path, "Reader1", "Antenna", "0");
            ...
            try
            {
                if (File.Exists(path))
                {
                    ...
                    if (items.Length == 12)
                    {
                        ...
                        reader.AntennaPower = Convert.ToInt32(INIOperation.INIGetStringValue(path, "Reader", "AntennaPower", "0"));
