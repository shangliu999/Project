...
{
    public class QRCodeBindingViewModel : ViewModelBase, IRFIDScan
    {
        private QRCodeBindingModel model;
        public QRCodeBindingModel Model
        {
            get { return model; }
            set
            {
        ...
        public QRCodeBindingViewModel(Action closeAction)
        {
            this.model = new QRCodeBindingModel();
            ...
            if (TextileModel != null)
            {
                ...
                if (!string.IsNullOrEmpty(QRCode) && QRCode.Length > 10)
                {
                    if (!isLock)
                    {
                        ...
                        var apiRtn = await ApiController.Instance.DoPost(ApiController.Instance.QRCodeBinding, requestParam);
                        if (apiRtn.ResultCode == 0)
                        {
                            this.model.BindingState = new Uri("../../Skins/Default/Images/img-bangdingchenggong.png", UriKind.RelativeOrAbsolute);
                        }
                        else
                        {
                            //this.model.BindingState = new Uri("../../Skins/Default/Images/img-bangdingshibai.png", UriKind.RelativeOrAbsolute);
                            if (apiRtn.OtherCode == "1")
                            {
                                string msg = string.Format("{0},是否先解除然后再绑定？", apiRtn.ResultMsg);
                                bool? isOK = EtexsysMessageBox.Show("提示", msg, MessageBoxButton.OKCancel, MessageBoxImage.Question);
                                if (isOK == true)
                                {
                                    apiRtn = await ApiController.Instance.DoPost(ApiController.Instance.QRCodeRemoveBinding, requestParam);
                                    if (apiRtn.ResultCode == 0)
                                    {
                                        this.model.BindingState = new Uri("../../Skins/Default/Images/img-bangdingchenggong.png", UriKind.RelativeOrAbsolute);
                                    }
                            ...
                            {
                                string msg = string.Format("{0},无法解除&绑定", apiRtn.ResultMsg);
                                EtexsysMessageBox.Show("提示", msg, MessageBoxButton.OK, MessageBoxImage.Information);
                            }
                        }
                        QRCode = string.Empty;
                        TextileModel = null;
